...
if (!MONGO_URI) {
  ...
  console.warn('[db] Missing MONGO_URI in environment. MongoDB will not connect.');
}
  ...
  expense: new mongoose.Schema(
    {
      ...
      expense_date: { type: String, default: () => new Date().toISOString().slice(0, 10) },
...
export const Models = {
  ...
  Expense: mongoose.models.Expense || mongoose.model('Expense', Schemas.expense),
  Collection: mongoose.models.Collection || mongoose.model('Collection', Schemas.collection),
  SentPayment: mongoose.models.SentPayment || mongoose.model('SentPayment', Schemas.sentPayment),
};
