...
export default function Dashboard({ user }) {
  ...
  const { data: cashLedger = [] } = useQuery({
    ...
    queryFn: () => (isAdmin ? base44.entities.CashLedger.list("-entry_date", 1000) : base44.entities.CashLedger.list()),
  });
  ...
  const staffTransfersTotal = staffTransfers.reduce((sum, t) => sum + (t.amount || 0), 0);

  // Staff-side balance: Transfers + Received - Sent - Expenses
  // (Admin formula is handled in staffSummary below and is not changed.)
  const balance = staffTransfersTotal + totalReceived - totalSent - totalExpenses;
  ...
  const staffSummary = isAdmin ? (() => {
    ...
    [...received, ...sent, ...expenses, ...staffTransfers].forEach(item => {
    ...
    transfers: staffTransfers.reduce((sum, t) => sum + (t.amount || 0), 0),
    received: totalReceived,
    sent: totalSent,
    expenses: totalExpenses,
    balance: staffTransfersTotal + totalReceived - totalSent - totalExpenses,
  }];
      ...
      {isAdmin && (
        <div className="space-y-2">
          <div className="flex items-center justify-between gap-3">
            <h3 className="font-heading font-semibold text-lg">Cash Added History</h3>
          ...
          {(() => {
            const sortedAll = [...(cashLedger || [])].sort((a, b) => {
              ...
              if (ad === bd) {
                ...
                return (bc ? new Date(bc).getTime() : 0) - (ac ? new Date(ac).getTime() : 0);
              }
            ...
            if (latest5.length === 0) {
              return <div className="text-muted-foreground text-sm">No cash entries found</div>;
            }
                    ...
                    <tbody>
                      {latest5.map((l) => (
                        <tr key={l.id} className="border-t border-border/50">
                          <td className="px-4 py-2 whitespace-nowrap font-medium">{formatCurrency(l.amount || 0)}</td>
                          <td className="px-4 py-2 whitespace-nowrap">{l.entry_date || "-"}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
          ...
          <div className="max-h-[70vh] overflow-y-auto pr-1">
            {(() => {
              ...
              if (sortedAll.length === 0) {
                return <div className="text-muted-foreground text-sm py-2">No cash entries found</div>;
              }
              ...
              <Card className="hover:shadow-lg transition-shadow border-t-4 border-t-primary">
                <CardContent className="p-5">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex justify-between items-center">
                      <span className="text-xs text-muted-foreground flex items-center gap-1">
                        <Wallet className="w-3 h-3 text-primary" />Transfers
                      </span>
                      <span className="font-semibold text-sm text-primary">{formatCurrency(s.transfers || 0)}</span>
                    </div>
